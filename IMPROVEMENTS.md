...
- [ ] A `vocabulary/core.yaml` file exists with at least 5 canonical actions and 3 assertions
- [ ] The linter validates BDD steps against this file and outputs specific suggestions, not just errors
- [ ] At least one step that uses a non-canonical synonym is auto-corrected with a suggestion
- [ ] Vocabulary file is versioned (semver in the YAML header)
- [ ] A CONTRIBUTING note explains how to propose a new vocabulary term
...
- [ ] Every generated spec has a `// DO NOT EDIT — generated by ai-driven-qa-platform` header
- [ ] A pre-commit hook rejects commits that modify files under `generated/`
- [ ] At least 3 Action Library helpers exist (`loginAs`, `navigateTo`, `assertText`)
- [ ] Generated code calls Action Library helpers, not raw `page.fill()`/`page.click()` for known actions
- [ ] Each generated spec includes a JSON comment block with `generatedAt`, `pageModelHash`, `plannerVersion`
...
- [ ] Two separate CLI entry points: `npm run generate` and `npm run heal`
- [ ] `heal` pipeline reads Playwright JSON reporter output as its input, not a `.feature` file
- [ ] `heal` pipeline's planner uses a different prompt (repair-focused) and temperature than `generate`
- [ ] A heal proposal is always a PR or patch file — never a direct write to `generated/`
- [ ] `generate` and `heal` pipelines are documented separately in README
...
- [ ] If `page.evaluate()` throws, no orphaned Chromium process remains (verified via `ps aux | grep chromium` before and after)
- [ ] A test that deliberately throws inside `evaluate()` confirms cleanup runs
...
- [ ] Running the pipeline 3 times with identical inputs produces identical `test-plan.json` output each time
- [ ] This is verified in a `test:planner-determinism` script that runs the planner 3x and diffs the outputs
...
- [ ] A spec path containing spaces (`/my projects/spec.ts`) runs correctly
- [ ] A spec path containing a single quote does not break the command
